...
Когда для ячейки сделан автоматический переход на другой экран, то для того что бы определить текущую строку, необходимо выполнить следующее:

```swift
// MARK: - Navigation
override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    guard let indexPath = tableView.indexPathForSelectedRow else { return }
    let selectPerson = contacts[indexPath.row]
    guard let personVC = segue.destination as? PersonViewController else { return }
    personVC.person = selectPerson
...
Метод `UITableViewDelegate.tableView(_:shouldHighlightRowAt:)` Спрашивает делегата, следует ли выделить указанную строку.

Метод `tableView(_:didHighlightRowAt:)` Сообщает делегату, что указанная строка была выделена.

Метод `tableView(_:didUnhighlightRowAt:)` Сообщает делегату, что выделение было удалено из строки в указанном индексном пути.

Метод `tableView.deselectRow(_:)` Отменяет выбор строки в указанном индексе, если он выбран.
...
Для кастомизацию заголовка секции можно использовать метод `viewForHeaderInSection`:
...
Так же, используя метод `viewForHeaderInSection`, возможно создать представление для загаловка, таким же образом как и для обычной ячейки

```swift
let cell = tableView.dequeueReusableCell(withIdentifier: "taskCellConstraints")
// если у нас есть отдельный view controller то кастим в него эту ячейку
let headerCell = cell as? HeaderTableViewCell
```

Для указания высоты секции используется метод `heightForHeaderInSection`



### Обновление

Метод `UITableView.reloadSections()` применяется к табличному представлению и позволяет заново сформировать и отобразить его на экране.
...
После активации режима редактирования, с левой стороны ячейки появляется кнопка. Ниже приведены методы позоляющие менять отображение этой кнопки:

```swift
// Меняет отображаемую картинку, либо вообще убирает ее
override func tableView(_ tableView: UITableView, editingStyleForRowAt indexPath: IndexPath) -> UITableViewCell.EditingStyle {
    .none
}

// Убирает пустое пространство с левой стороны 
override func tableView(_ tableView: UITableView, shouldIndentWhileEditingRowAt indexPath: IndexPath) -> Bool {
    false
}
```

Для того что бы с правой стороны начали отображаться "бургеры" необходимо добавить метод `moveRowAt`
